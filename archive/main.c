...
==STAT=====================================
  ...
  angle_y 0x0B 
  angle_z 0x0C 
  cont_l  0x0D // Contact status, left foot
  cont_r  0x0E // Contact status, right foot
==CFG=====================================
  dt          0x00  // loop time
...
// ==STAT=========================
float32 *temperature = &STAT[0]; // filtered angle
...
// Get time since boot in us
uint64_t us_since_boot(){
    uint64_t _time = to_us_since_boot(get_absolute_time());
...
void setup() {
    ...
    if (data[0] != ISM330DHCX_ID) {
        ...
        uint64_t start_time = us_since_boot();
        t_final = start_time;
        while (_initiating){
            readIMU(); // Main CF function inside
            if ((us_since_boot()-start_time)>1000000) {
                if (debug){
                    printf("Complementary filter initiated.\n");
                }
            _initiating = false;
        }
        while ((us_since_boot()-t_final) < (*dt).floatingPoint) {} // Keep loop at dt
        t_final = us_since_boot();
        }
    }
}
...
void dump(){  
    ...
    // printf("\r\n");
    // for (int i=0; i<9; i++){
    //     printf("%c%c%c%c%c%c", rtnPkt[6*i], rtnPkt[6*i+1], rtnPkt[6*i+2], rtnPkt[6*i+3], rtnPkt[6*i+4], rtnPkt[6*i+5]);
    // }
    printf("%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c", 
            rtnPkt[0],  rtnPkt[1],  rtnPkt[2],  rtnPkt[3],  rtnPkt[4],  rtnPkt[5],  rtnPkt[6],  rtnPkt[7],  rtnPkt[8],
            rtnPkt[9],  rtnPkt[10], rtnPkt[11], rtnPkt[12], rtnPkt[13], rtnPkt[14], rtnPkt[15], rtnPkt[16], rtnPkt[17],
            rtnPkt[18], rtnPkt[19], rtnPkt[20], rtnPkt[21], rtnPkt[22], rtnPkt[23], rtnPkt[24], rtnPkt[25], rtnPkt[26],
            rtnPkt[27], rtnPkt[28], rtnPkt[29], rtnPkt[30], rtnPkt[31], rtnPkt[32], rtnPkt[33], rtnPkt[34], rtnPkt[35],
            rtnPkt[36], rtnPkt[37], rtnPkt[38], rtnPkt[39], rtnPkt[40], rtnPkt[41], rtnPkt[42], rtnPkt[43], rtnPkt[44],
            rtnPkt[45], rtnPkt[46], rtnPkt[47], rtnPkt[48], rtnPkt[49], rtnPkt[50], rtnPkt[51], rtnPkt[52], rtnPkt[53]);
    // printf("%s",rtnPkt);
    // Serial.write(rtnPkt,53);
}
...
int main() {
    ...
    while (true)
    {
        readIMU();
        dump();
        loop = (us_since_boot()-t_final);
        // printf("Loop Time: %10.2f\r\n", loop);
        if (loop > (*dt).floatingPoint){
